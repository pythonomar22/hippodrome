...
export default function Chessboard() {
  ...
  function grabPiece(e: React.MouseEvent) {
    ...
    if (element.classList.contains("chess-piece") && chessboard) {
      setGridX(Math.floor((e.clientX - chessboard.offsetLeft) / 150));
      setGridY(
        Math.abs(Math.ceil((e.clientY - chessboard.offsetTop - 600) / 150))
      );
  ...
  function dropPiece(e: React.MouseEvent) {
    const chessboard = chessboardRef.current;
    if (activePiece && chessboard) {
      const x = Math.floor((e.clientX - chessboard.offsetLeft) / 150);
      const y = Math.abs(
        Math.ceil((e.clientY - chessboard.offsetTop - 600) / 150)
      );
